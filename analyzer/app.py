import connexion, json, datetime, logging.config, yaml, pykafka, os
...
def get_listings(index): 
    ...
    consumer.stop()
    logger.debug("Consumer stopped for get-listings successfully!")
...
def get_bids(index): 
    ...
    for msg in consumer:
        ...
        if data["type"] == "bids":
            ...
            counter += 1
    consumer.stop()
    logger.debug("Consumer stopped for get-bids successfully!")
    return {"message": f"No message at index {index}!"}, 404
...
def get_stats():
    ...
    for msg in consumer:
        ...
        if data["type"] == "listings":
            ...
            bids_counter += 1
    consumer.stop()
    logger.debug("Consumer stopped for get-stats successfully!")
